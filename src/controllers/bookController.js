...
const createBook = async function (req, res) {
  try {
    ...
    if (!userCheck) {
      return res.status(400).send({ status: false, msg: "user dosnt exist with this user id" })
    }
...
const getBooks = async function (req, res) {
  try {
    ...
    let search = await bookModels.find(filterObject).select({ _id: 1, title: 1, excerpt: 1, userId: 1, category: 1, reviews: 1, releasedAt: 1 ,bookCover:1});
